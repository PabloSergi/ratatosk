...
The token names the account whose scrapers this server speaks for — copy it from **token for MCP** in the
web view. Without accounts it is not needed; with accounts, a missing one is refused rather than
quietly showing somebody else's scrapers. See [accounts](accounts.md).

## Building a scraper — five tools, used in a loop
...
| `save(name, …)` | freezes it as a scraper, but only after running it and getting rows |
...
scrapers return nothing and call it success.

## Consuming data — two tools, no selectors

| Tool | What it answers |
|---|---|
| `scrapers()` | which scrapers exist and what columns each returns |
...
no scraper named "flats". Known scrapers: books, jobs
...
Drives a real server over stdio through the whole cycle — open, look, try, probe, save, scrapers, fetch —
against a live site, and finishes by asking for a scraper that does not exist to see that the refusal is
